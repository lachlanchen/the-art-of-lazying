...
### Why a full reconnect still looked US-like

After correcting XRDP and reconnecting completely, the new connection logged:
...
Nevertheless, real typing remained US-like. Inspection on the Mac relay found:

```text
Windows App keyboard mode: Unicode (1)
Current macOS input source: com.apple.keylayout.ABC
```

That result is internally consistent. Unicode mode sends the printable
character generated by the Mac, so XRDP's physical JIS keymap cannot turn a US
`Shift+7` character into a JIS apostrophe without also corrupting intentional
`&` input. At this point, more Ubuntu remapping is the wrong fix.

The Mac already had these selectable Japanese sources enabled:

```text
com.apple.inputmethod.Kotoeri.RomajiTyping.Japanese  # Hiragana via Romaji
com.apple.inputmethod.Kotoeri.RomajiTyping.Roman     # Roman mode
com.apple.inputmethod.Kotoeri.KanaTyping.Japanese    # direct Kana typing
```

Use the generic helper saved with this guide to inspect or select them:

- [scripts/macos-input-source.swift](./scripts/macos-input-source.swift)

```bash
swift scripts/macos-input-source.swift current
swift scripts/macos-input-source.swift list

# Japanese text through Romaji conversion
swift scripts/macos-input-source.swift select \
  com.apple.inputmethod.Kotoeri.RomajiTyping.Japanese

# Return to plain ABC input
swift scripts/macos-input-source.swift select com.apple.keylayout.ABC
```

Selecting a Japanese source changes text input; it is not the same as making a
remotely injected ANSI-like keyboard become physical JIS hardware. Test it in a
Mac text field before reopening RDP, and retain `ABC` as the immediate rollback.
Apple documents both [adding Japanese Kana or Romaji input](https://support.apple.com/guide/japanese-input-method/set-up-a-japanese-input-source-jpim10267/mac)
and [switching Japanese modes, including Kana and Eisu](https://support.apple.com/guide/japanese-input-method/switch-to-a-japanese-input-source-jpimf6ffb247/mac).

For exact physical JIS punctuation, the Windows relay route may be more
reliable because the Windows RDP client can send Scancodes and modifiers while
the Ubuntu server applies its separate type-7 compatibility map. That route
must be tested independently; success on the Mac route does not prove the
Windows route and vice versa.
