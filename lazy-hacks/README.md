...
- [claude-cli](./claude-cli/README.md): Claude Code shortcuts in two flavors: native-delegating `claude-shell.sh`, and codex-style `claude`/`clauder`/`claudemv` wrappers with enforced full access, a folder-scoped resume picker, and session migration with rollback.
